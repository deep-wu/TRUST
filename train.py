...
from trust.trust_loss import *
from trust.st_opt import st_opt

from torch.autograd import Variable
from torch.utils.tensorboard import SummaryWriter
from torchvision import datasets, transforms
from utils.utils import Logger, random_seed, save_checkpoint

warnings.filterwarnings("ignore")

parser = argparse.ArgumentParser(description="PyTorch CIFAR TRUST Defense")
parser.add_argument(
...
parser.add_argument(
    "--data-path", type=str, default="./cifar10", help="where is the dataset CIFAR-10"
)
parser.add_argument(
    "--batch-size",
    type=int,
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # 定义 closure
        def closure():
            optimizer.zero_grad()  # 每次都清零梯度
            loss, ce_u, ce_clean, ce_v = tri_loss(
                model=model,
                x_natural=data,
                y=target,
                optimizer=optimizer,
        ...
        optimizer.step(closure)  # 传入 closure
